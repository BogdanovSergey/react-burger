...
export const ResetPasswordPage = () => {
    ...
    useEffect(()=>{
        ...
        if(!loggedIn || localStorage.getItem('refreshToken') || getCookie('token')) {
        ...
        <>
            <div className={css.login_box}>
                ...
                <form className={css.login_fields} onSubmit={handleSubmit}>
                    ...
                    <Input type={"password"} placeholder={"Введите новый пароль"} value={password} onChange={(event)=>setPassword(event.target.value)}/>
                    <Input type={"text"} placeholder={"Введите код из письма"} value={token} onChange={(event)=>setToken(event.target.value)}/>
